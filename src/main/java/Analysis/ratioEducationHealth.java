...
public class ratioEducationHealth implements Analysis {
	
	public void performComputation() {
		// populate the result object with the computed values
	}
	
	private double getGovernmentExpenditure() {
	...
	private double getHealthExp() {
		String urlString = String.format("http://api.worldbank.org/v2/country/%s/indicator/SH.XPD.CHEX.GD.ZS?date&format=json", "all");
		System.out.println(urlString);
		double expenditureForYear = 0;
		try {
			URL url = new URL(urlString);
	...
	private double getRatio() {
		return getGovernmentExpenditure() / getHealthExp();
	}
